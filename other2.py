...
train_indices_, dev_indices = [], []
for train_index, dev_index in kf.split(labels):
    train_indices_.append(train_index)
    dev_indices.append(dev_index)
for num in range(10):
    train_index, dev_index = train_indices_[num], dev_indices[num]
    sentences_train_, sentences_dev = sentences[train_index], sentences[dev_index]
    tags_train_, tags_dev = tags[train_index], tags[dev_index]
    labels_train_, labels_dev = labels[train_index], labels[dev_index]
...
def test_epoch_1(dataset):
    """Testing or valid."""
    _batch_size = 500
    fetches = [correct_prediction_,accuracy, cost]
    _y = dataset.y
    data_size = _y.shape[0]
    batch_num = int(data_size / _batch_size)
    start_time = time.time()
    ...
    for i in range(batch_num):
        X_batch,X_tag_batch, y_batch = dataset.next_batch(_batch_size)
        feed_dict = {X_inputs:X_batch,X_tag_inputs:X_tag_batch, y_inputs:y_batch, lr:1e-5, batch_size:_batch_size, keep_prob:1.0}
        prediction_current, _acc, _cost = sess.run(fetches, feed_dict)
        n=0
    ...
    mean_acc= _accs / batch_num     
    mean_cost = _costs / batch_num
    return prediction_result,mean_acc, mean_cost
...
# testing
print ('**TEST RESULT:')
array, test_acc, test_cost = test_epoch_1(data_test)
print( '**Test %d, acc=%g, cost=%g' % (data_test.y.shape[0], test_acc, test_cost))
print(array)

print('**Save the test result at**')
...
prediction_test,test_acc_, test_cost_, = test_epoch_1(data_test)
